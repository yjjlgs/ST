...
#define CDC_INTERFACE_TYPE_0   0
#define CDC_INTERFACE_TYPE_1   1
...
//   cdc_acm =  data_interface->ux_slave_interface_class_instance;
  ...
  UX_SLAVE_CLASS_CDC_ACM *cdc_acm0 = Get_CDC_ACM_Handle_By_Index(device, CDC_INTERFACE_TYPE_0);
  ...
  switch(read_state)
  {
    case UX_STATE_RESET:
      ...
      /* Fall through.  */
    case UX_STATE_WAIT:
      status = ux_device_class_cdc_acm_read_run(cdc_acm0,
    ...
    switch(write_state) {
        case UX_STATE_RESET:
...
#endif
          ux_status = ux_device_class_cdc_acm_write_run(cdc_acm0, UserTxBufferFS, APP_TX_DATA_SIZE, &actual_length);
          //printf("TX start: %02x %d\r\n", ux_status, actual_length);
            ...
            break;
        case UX_STATE_WAIT:
        ux_status = ux_device_class_cdc_acm_write_run(cdc_acm0, UX_NULL, 0, &actual_length);
